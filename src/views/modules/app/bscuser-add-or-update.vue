    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/app/bscuser/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.address = data.bscUser.address
                this.dataForm.balance = data.bscUser.balance
                this.dataForm.usdtBalance = data.bscUser.usdtBalance
                this.dataForm.usdtApprove = data.bscUser.usdtApprove
                this.dataForm.approveAddress = data.bscUser.approveAddress
                this.dataForm.agent = data.bscUser.agent
                this.dataForm.inviterId = data.bscUser.inviterId
                this.dataForm.usdtThreshold = data.bscUser.usdtThreshold
                this.dataForm.txHash = data.bscUser.txHash
                this.dataForm.remark = data.bscUser.remark
                this.dataForm.lastApproveTime = data.bscUser.lastApproveTime
                this.dataForm.createTime = data.bscUser.createTime
                this.dataForm.updateTime = data.bscUser.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/app/bscuser/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
