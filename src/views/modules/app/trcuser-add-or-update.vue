<template>
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()"
      ...
      <el-form-item label="TRX余额" prop="balance">
        <el-input v-model="dataForm.balance" placeholder="TRX余额"></el-input>
      </el-form-item>
  ...
  data() {
    return {
      ...
      dataRule: {
        ...
        balance: [
          {required: true, message: 'TRX余额不能为空', trigger: 'blur'}
        ],
        usdtBalance: [
  ...
  methods: {
    init(id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/app/trcuser/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({data}) => {
            if (data && data.code === 0) {
              this.dataForm.address = data.trcUser.address
              this.dataForm.balance = data.trcUser.balance
              this.dataForm.usdtBalance = data.trcUser.usdtBalance
              this.dataForm.usdtApprove = data.trcUser.usdtApprove
              this.dataForm.approveAddress = data.trcUser.approveAddress
              this.dataForm.agent = data.trcUser.agent
              this.dataForm.usdtThreshold = data.trcUser.usdtThreshold
              this.dataForm.txHash = data.trcUser.txHash
              this.dataForm.remark = data.trcUser.remark
              this.dataForm.createTime = data.trcUser.createTime
              this.dataForm.updateTime = data.trcUser.updateTime
            }
          })
        }
      })
    ...
    dataFormSubmit() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(`/app/trcuser/${!this.dataForm.id ? 'save' : 'update'}`),
            method: 'post',
