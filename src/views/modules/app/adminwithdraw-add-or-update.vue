    ...
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/app/adminwithdraw/info/${this.dataForm.id}`),
              method: 'get',
              params: this.$http.adornParams()
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.dataForm.adminId = data.adminWithdraw.adminId
                this.dataForm.amount = data.adminWithdraw.amount
                this.dataForm.type = data.adminWithdraw.type
                this.dataForm.txHash = data.adminWithdraw.txHash
                this.dataForm.from = data.adminWithdraw.from
                this.dataForm.to = data.adminWithdraw.to
                this.dataForm.status = data.adminWithdraw.status
                this.dataForm.details = data.adminWithdraw.details
                this.dataForm.remark = data.adminWithdraw.remark
                this.dataForm.createTime = data.adminWithdraw.createTime
                this.dataForm.updateTime = data.adminWithdraw.updateTime
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/app/adminwithdraw/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
